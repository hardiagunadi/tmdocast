 ...
 */
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (array_key_exists('csrf_token', $_POST) && isset($_POST['csrf_token']) && dalo_check_csrf_token($_POST['csrf_token'])) {
            ...
            if ($userExists) {
                ...
                $logAction .= "Failed adding PPPoE user (exists) on page: ";
            } elseif (empty($username) || empty($password) || empty($planName)) {
                $failureMsg = "Username, password, dan paket wajib diisi.";
                $logAction .= "Failed adding PPPoE user (missing data) on page: ";
            } else {
                ...
                foreach ($target_nas as $nas) {
                    $pool_network = !empty($nas['pppoe_pool_network']) ? $nas['pppoe_pool_network'] : '172.16.0.0';
                    $pool_cidr = !empty($nas['pppoe_pool_cidr']) ? intval($nas['pppoe_pool_cidr']) : 20;
                    $pppoe_ip = mikrotik_next_pppoe_ip($dbSocket, $configValues, $username, $nas['id'], $pool_network, $pool_cidr);
                    if (empty($pppoe_ip)) {
                        $pppoe_ip_sql = "NULL";
                    } else {
                        $pppoe_ip_sql = "'" . $dbSocket->escapeSimple($pppoe_ip) . "'";
                    }

                    $sql = sprintf("INSERT INTO %s (username, service_type, nas_id, plan_name, expiration_date, ip_address, creationdate, creationby, updatedate, updateby)
                                    VALUES ('%s', 'pppoe', %d, '%s', %s, %s, '%s', '%s', '%s', '%s')",
                ...
                $successMsg = "User PPPoE berhasil dibuat: <b>$username_enc</b>";
                $logAction .= "Added PPPoE user on page: ";
            }
    ...
    open_form();
    open_fieldset(array("title" => "Data PPPoE"));
